...
def is_lines_valid(list_of_lines):
  for line in list_of_lines:
    if int(line) == 0:
      print(f"Line 0 is invalid. Lines need to be starting from 1")
      return False
    elif int(line) > ROWS:
      print(f"The line {line} entered exceeds the capacity of at most line {ROWS}")
      return False
  return True

def is_total_bet_valid(balance, total_bet_amount):
  if balance < total_bet_amount:
    print(f"Your total bet amount of ${total_bet_amount} exceeds your current balance of ${balance}")
    return False
  return True

def get_num_lines(balance, bet_amount):
  while True:
    lines = input("Enter lines to bet on (1-3), separated by spaces (eg. 1 2): ")
    lines_without_space = re.sub(r" ", r"", lines)
    if not is_input_digit("Number of lines", lines_without_space, False):
      continue
