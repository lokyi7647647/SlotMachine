"""
# tests / unit / get betting lines test

Test the get_bet_lines function in reading valid/invalid betting lines
"""
...
def test_get_bet_valid(monkeypatch, capsys, balance_and_bet):
    # Mock reading the betting lines from stdin
...
def test_get_bet_invalid_zeroth_line(monkeypatch, capsys, balance_and_bet):
    # slot machine can only have lines 1 to 3, line 0 is invalid
...
def test_lines_contain_bigger_than_3(monkeypatch, capsys, balance_and_bet):
    # slot machine can only have line 1 to 3, line 4 is invalid
    monkeypatch.setattr("sys.stdin", StringIO('1 4 3'))
    balance, bet_amount = balance_and_bet
    with pytest.raises(EOFError):
        get_bet_lines(balance, bet_amount)
    captured = capsys.readouterr()
    assert captured.err == "The line '4' entered exceeds the maximum available line, which is '3'\n"
...
def test_total_bet_exceeds_balance(monkeypatch, capsys):
    ...
    # two lines from input '1 2', each cost $6, so a total bet is $12
...
def test_total_bet_equals_balance(monkeypatch, capsys):
    ...
    # two lines from input '1 2', each cost $5, so a total bet is $10
    assert captured.out == (
        "Enter lines to bet on (1-3), separated by spaces (eg. 1 2): "
